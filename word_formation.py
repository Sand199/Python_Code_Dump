...
def updateHand(hand, word):
    ...
    return {k: v - sum(1 for l in word if l == k) for k, v in hand.items()}
...
def isValidWord(word, hand, wordList):
    ...
    return (
        not {1 for k in word if sum(1 for l in word if l == k) > hand.get(k, 0)}
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while sum(v for v in hand.values()):
